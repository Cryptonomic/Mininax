...
module Styles = {
  ...
  let input = index =>
    style([
      ...
      media("(max-width: 1300px)", [flex(`num(1.0))]),
    ]);
  ...
  let changeBtn = index =>
    style([
      ...
      fontFamily(`custom("'Perfect DOS VGA 437 Win', sans-serif")),
      media("(max-width: 1300px)", [marginLeft(px(30))]),
    ]);
...
let make =
  ...
  let onChange = event => changeId(ReactEvent.Form.target(event)##value);
  ...
  <div className=Styles.container>
    <input
      ref={ReactDOMRe.Ref.callbackDomRef(ref => doEffectWhenRefChanges(ref))}
      value=searchVal
      onChange
      onKeyDown
